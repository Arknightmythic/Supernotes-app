...
func (n *notebookRepository) GetAll(ctx context.Context) ([]*entity.Notebook, error) {
	rows, err := n.db.Query(ctx, `SELECT id, name, parent_id, created_at, updated_at FROM notebook WHERE is_deleted = false ORDER BY name`,)
	...
	for rows.Next(){
		...
		result = append([]*entity.Notebook{&notebook}, result...) //agar ascending
	}
	return result, nil
}
